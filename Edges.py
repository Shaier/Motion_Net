...
# Import the original image
input_path = os.getcwd() # This is where the code is running.
filepath = os.path.join(input_path,'../Test_Images/salsa2.jpg')  # The images are located in a folder called 'Test_Images' within the root directory from where the code runs.

Image_orig = mh.imread(filepath) # Read the image.
# To convert the color image to grayscale
if Image_orig.ndim ==3:
    Image_orig_grey = mh.colors.rgb2grey(Image_orig)  # Image_orig is color image. ###########Change to 96x96 here if possible@@@@@@@@@@@@@@@@@@@
else:
    Image_orig_grey = Image_orig
...
plt.imshow(image)
#######################################################
'''Clean version to get the image, show, and save'''
